...
void loop()
{
   if (pms.read(data)){ 
    ...
    String full_url;
    int httpCode;
    full_url = SERVER_ADDRESS + "am2302.php?temperature=" + temperaturaEnGradosCelsius + "&humidity=" + humedad + "&idsensor=1";
    http.begin(full_url);
    httpCode = http.GET();
    http.end();

    full_url = SERVER_ADDRESS + "pms7003.php?pm2=" + data.PM_AE_UG_2_5 + "&pm10=" + data.PM_AE_UG_10_0 + "&idsensor=3";
    http.begin(full_url);
    httpCode = http.GET();
    http.end(); 

    if(data.PM_AE_UG_2_5 > 37){
      HTTPClient http;
      String full_url;
      int httpCode;
      full_url = SERVER_ADDRESS + "email.php?valor=" + data.PM_AE_UG_2_5 + "&pm2=true";
      http.begin(full_url);
      httpCode = http.GET();
      http.end();        
    }

    if(data.PM_AE_UG_10_0 > 75){
      HTTPClient http;
      String full_url;
      int httpCode;
      full_url = SERVER_ADDRESS + "email.php?valor=" + data.PM_AE_UG_10_0 + "&pm10=true";
      http.begin(full_url);
      httpCode = http.GET();
      http.end();        
    }
